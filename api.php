...
requireAuth();
sendSecurityHeaders();
// Валідація action — тільки дозволені значення
$action = $_GET['action'] ?? '';
$allowedActions = ['list_logs','analyze_threats','list_threat_files','check_status','detect_gateway','save_gateway','peek_file'];
...
switch ($action) {
    ...
    // ── Діагностика: перші рядки файлу ─────────────────────────────────────
    case 'peek_file':
        header('Content-Type: application/json');
        $sid  = (int)($_GET['sid'] ?? 0);
        $file = $_GET['file'] ?? '';
        if (!$sid || !$file) { echo json_encode(['error' => 'No sid/file']); exit; }
        ...
        $conn = sshConnect($srv);
        if (!$conn['success']) { echo json_encode(['error' => $conn['error']]); exit; }

        // Перші 20 рядків файлу для діагностики
        $lines = sshExec($conn, "head -20 " . escapeshellarg($file) . " 2>/dev/null");
        if (empty(trim($lines))) {
            // Спробуємо з sudo
            $lines = sshExec($conn, "sudo head -20 " . escapeshellarg($file) . " 2>/dev/null");
        }
        $sz  = (int)trim(sshExec($conn, "wc -c " . escapeshellarg($file) . " 2>/dev/null | awk '{print $1}'"));
        $lc  = (int)trim(sshExec($conn, "wc -l " . escapeshellarg($file) . " 2>/dev/null | awk '{print $1}'"));
        $perm = trim(sshExec($conn, "ls -la " . escapeshellarg($file) . " 2>/dev/null | awk '{print $1, $3, $4}'"));
        ...
        echo json_encode([
            ...
            'lines' => $lc,
            'perms' => $perm,
            'head'  => $lines ?: '(порожньо або немає доступу)',
        ]);
        exit;

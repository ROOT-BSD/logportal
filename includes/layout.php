...
/**
 * Відправляє HTTP security headers для захисту від XSS, clickjacking, MIME-sniffing
 * Викликати на початку кожної сторінки до будь-якого виводу
 */
function sendSecurityHeaders(): void {
    ...
    // XSS-захист (старі браузери)
    header('X-XSS-Protection: 1; mode=block');
    // HTTPS only (якщо HTTPS)
    if (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') {
        header('Strict-Transport-Security: max-age=31536000; includeSubDomains');
    }
    // Referrer Policy
    header('Referrer-Policy: strict-origin-when-cross-origin');
    // Permissions Policy — вимикаємо непотрібні API
    header('Permissions-Policy: camera=(), microphone=(), geolocation=(), payment=()');
    // Content Security Policy
    // script-src: 'self' + 'unsafe-inline' потрібен для inline JS (theme toggle тощо)
    // Для повного захисту — перенести inline JS у зовнішні файли та прибрати 'unsafe-inline'
    header(
        "Content-Security-Policy: " .
        "default-src 'self'; " .
...
<footer class="site-footer"><div class="container">
  <div class="footer-logo">LogPortal v2.1.0</div>

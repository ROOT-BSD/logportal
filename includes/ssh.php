...
function sshExec(array $c, string $cmd): string {
    if (!empty($c['conn'])) {
        ...
        if (!$s) return '';
        // Для коректного читання великих файлів треба читати обидва потоки
        $se = @ssh2_fetch_stream($s, SSH2_STREAM_STDERR);
        stream_set_blocking($s,  true);
        stream_set_blocking($se, true);
        // Таймаут 5 хвилин для читання великих файлів
        stream_set_timeout($s,  300);
        stream_set_timeout($se, 300);
        $out = stream_get_contents($s);
        // Читаємо stderr щоб не блокувати буфер (але не включаємо в результат)
        @stream_get_contents($se);
        fclose($se);
        fclose($s);
        return (string)$out;
    }

...
if (!$dbError && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'login') {
        ...
        if (!$error) {
            // Якщо треба змінити пароль — залишаємось на index.php з формою зміни
            if (!empty($_SESSION['force_pw_change'])) {
                header('Location: index.php?force_pw=1');
            } else {
                header('Location: servers.php');
            }
            exit;
        ...
        if (!$uid) {
            ...
            $tab = 'change_pw';
        } elseif (preg_match('/^[a-z0-9]+$/i', $newPw)) {
            $error = 'Пароль повинен містити літери та цифри або спецсимволи';
            $tab = 'change_pw';
        } else {
            getDB()->prepare("UPDATE users SET password=?, force_password_change=0 WHERE id=?")
...
if (isset($_GET['force_pw'])) $tab = 'change_pw';
...
sudo chmod 750 /var/www/html/data</pre>
        ...
        <?php else: ?>
          ...
          <!-- Tabs -->
          <?php if ($tab === 'change_pw'): ?>
          <!-- Force password change panel -->
          <div style="text-align:center;margin-bottom:20px">
            <div style="font-size:2rem;margin-bottom:8px">🔐</div>
            <div style="font-size:1rem;font-weight:700;color:var(--blue-dark)">Зміна пароля за замовчуванням</div>
            <div style="font-size:.82rem;color:var(--gray-mid);margin-top:6px">
              З міркувань безпеки необхідно встановити власний пароль перед початком роботи
            </div>
          </div>
          <div id="panel-change-pw" class="auth-panel active">
            <form method="POST" autocomplete="off">
              <input type="hidden" name="action" value="change_password">
              <div class="form-group">
                <label class="form-label">Новий пароль <span class="req">*</span></label>
                <input type="password" name="new_password" class="form-control"
                       placeholder="Мін. 8 символів, літери + цифри/спецсимволи"
                       autocomplete="new-password" required>
                <div class="form-hint">Мінімум 8 символів, не може складатись лише з літер або цифр</div>
              </div>
              <div class="form-group">
                <label class="form-label">Повторіть пароль <span class="req">*</span></label>
                <input type="password" name="new_password2" class="form-control"
                       placeholder="Повторіть пароль"
                       autocomplete="new-password" required>
              </div>
              <button type="submit" class="btn btn-primary btn-block btn-lg" style="margin-top:20px;background:#2e7d32;border-color:#2e7d32">
                ✓ Встановити пароль
              </button>
            </form>
          </div>
